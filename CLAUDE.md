...
- **HTML Generator Email Fallback (2026-04-07)**: The UI email display in `generate_html.js` was checking ONLY `item.campPOCEmail`, but the extraction engine saved emails to `item.email`. The `campPOCEmail` field is not populated by the extraction engine, so 500+ schools showed "Check site for contact info" even when a valid email existed. Fix: both card-level (`contactEmail`) and modal-level (`emailHtml`) must fallback through `item.campPOCEmail || item.email || ''`. Never check only `campPOCEmail` — it's a migration artifact that never gets populated by the V10 extractor.
- **"Lost Tiers" Data Loss Pattern (2026-04-07)**: 36 schools across the database (Alabama, Arizona, Appalachian State, etc.) had populated `dates` and `cost` fields but EMPTY `campTiers: []`. This means the extraction engine initially found and saved tiers, but a subsequent data merge/normalize script overwrote the `campTiers` array while preserving the flat `dates`/`cost` strings. The extraction engine's `extractDataFromText()` DOES populate `campTiers` — the data loss lives in the merge/finalize pipeline. Recovery requires: (1) checking if `campTiers` exists but is empty while `campTiers` has dates/costs, (2) re-running extraction OR reconstructing tiers from the flat strings. NEVER overwrite `campTiers` with `[]` during data normalization — preserve existing array data.
- **Name-near-Email Extraction (2026-04-07)**: When pages say "Email Drew Bishop at Drew.Bishop@athletics.utexas.edu", the `harvestEmails()` function only captured the email address and ignored the surrounding name. This is a systemic gap — every school's POC name that appears adjacent to their email is being lost. Added `extractNameNearEmail()` function in `extraction_engine.js` with patterns for common contextual forms ("emailing X at", "contact: X |", "X — email@"). The contact logic at the extraction merge point now calls this as a fallback when `pointOfContact` is N/A.
