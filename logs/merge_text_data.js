...
for (let line of lines) {
    ...
    let campUrl = urlMatch ? urlMatch[1] : (parts[2].includes('http') ? parts[2] : null);
    let dates = parts[3];
    let cost = parts[4];
    let details = parts[5];
    ...
    if (entry) {
        ...
        const isBetterData = (dates !== 'TBA' && (entry.dates === 'TBA' || entry.dates.includes('No 2026') || entry.dates.includes('International Affairs') || entry.dates.includes('Piano')));
        const isPlaceholder = entry.dates === 'TBA' || entry.dates.includes('No 2026');

        if (isBetterData || isPlaceholder) {
            console.log(`Merging ${entry.university} [${dates}] over [${entry.dates.substring(0, 30)}...]`);
