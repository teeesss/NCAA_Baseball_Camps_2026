...
async function audit() {
  ...
  for (const record of data) {
    ...
    if (record.cost && record.cost !== "TBA") {
      ...
      if (match) {
        ...
        if (val < SUSPICIOUS_THRESHOLDS.MIN_COST && val > 0) {
          issues.push(
            `SUSPICIOUS_COST: ${record.cost} (< $${SUSPICIOUS_THRESHOLDS.MIN_COST})`,
          );
        }
      }
    }
    ...
    if (record.dates && record.dates !== "TBA") {
      if (SUSPICIOUS_THRESHOLDS.PLACEHOLDER_PATTERN.test(record.dates)) {
        issues.push(
          `PLACEHOLDER_DATES: Detected "20th" or "XX/20" pattern: ${record.dates}`,
        );
      }
      const dateCount = record.dates.split("|").length;
      if (dateCount > SUSPICIOUS_THRESHOLDS.MAX_DATES_LOGICAL) {
        issues.push(
          `OVERFLOW_DATES: Found ${dateCount} separate dates (likely generic calendar scrape)`,
        );
      }
    }
    ...
    if (BANNED_KEYWORDS.some((k) => combined.toLowerCase().includes(k))) {
      issues.push(
        `BLACK_KEYWORDS: Source linked to third-party aggregator patterns: ${record.campUrl}`,
      );
    }
    ...
    if (
      record.university.includes("South Dakota State") &&
      record.dates.includes("SMU")
    ) {
      issues.push(
        `CROSS_CONTAMINATION: Contains SMU data in South Dakota State record.`,
      );
    }
  ...
  if (suspicious.length === 0) {
    ...
    console.log(
      "Run node src/tests/scrub_bad_data.js to clear these if they are false positives.",
    );
  }
}
