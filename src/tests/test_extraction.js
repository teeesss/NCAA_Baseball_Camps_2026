    ...
    try {
      ...
      const providers = [
        ...
        },
        {
          ...
          search: async (q) => {
            ...
            return await p.evaluate(() =>
              Array.from(
                document.querySelectorAll("a.result-header, h3 a, #results a"),
              ).map((a) => a.href),
            );
          },
        },
        {
          name: "Ask",
          search: async (q) => {
            await p.goto(`https://www.ask.com/web?q=${encodeURIComponent(q)}`, {
            ...
            return await p.evaluate(() =>
              Array.from(
                document.querySelectorAll(
                  ".PartialSearchResults-item-title-link, a.result-link, h3 a, .compTitle a",
                ),
              ).map((a) => a.href),
            );
          },
      ...
      for (let q of queries) {
        ...
        for (let step = 0; step < providers.length; step++) {
          ...
          try {
            ...
            log(
              `      ✕ ${provider.name} error: ${e.message.substring(0, 30)}`,
            );
          }
          await delay(1200);
        }
      ...
      searchLinks = [...new Set(searchLinks)].filter(
        (l) =>
          ...
          !l.includes("bing.com/search") &&
          !l.includes("search.yahoo"),
      );
      ...
      for (let item of scored) {
        try {
          log(`\n   -> Trying: ${item.url}`);
          const resp = await p.goto(item.url, {
            waitUntil: "domcontentloaded",
            timeout: 15000,
          });
          ...
          if (contam) {
            log(
              `      ✕ Contamination: Match found for school "${contam}" in title "${title}"`,
            );
            continue;
          }
          ...
          if (campTiers.length > 0) {
            ...
            if (camp.details)
              log(`      [Details]: ${camp.details.substring(0, 80)}...`);

            log(`\n      [Extracted Tiers]:`);
            campTiers.forEach((t) =>
              log(`        ✓ ${t.name} | Dates: ${t.dates} | Cost: ${t.cost}`),
            );
            break;
          } else {
