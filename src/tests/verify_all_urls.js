...
async function auditUrlContent(url, schoolName) {
  return new Promise((resolve) => {
    ...
    let settled = false;
    const settle = (ok, reason = '') => { if (!settled) { settled = true; resolve({ ok, reason }); } };

    let parsed;
    try { parsed = new URL(url); } catch (_) { return settle(false, 'invalid_url'); }
    ...
    const options = {
      ...
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; NCAABot/1.3; +https://bmwseals.com/)' },
      timeout: 15000
    };

    const req = lib.request(options, (res) => {
        if (res.statusCode >= 400) return settle(false, `status_${res.statusCode}`);
        
        let body = '';
        res.setEncoding('utf8');
        res.on('data', (chunk) => {
            body += chunk;
            if (body.length > 32768) { // 32KB threshold for deep scans
                req.destroy();
                analyze(body);
            }
        });
        ...
        function analyze(html) {
            ...
            const aliases = (ABBREVIATION_MAP[schoolName] || []).map(a => a.toLowerCase());
            
            // 1. Basic relevance (Baseball/Camp)
            const baseballKeywords = ['baseball', 'diamond', 'ballclub', 'ball', 'dugout', 'inning'];
            ...
            if (!hasCamp && !url.includes('ryzer.com')) return settle(false, 'missing_camp_context');
            ...
            if (isContaminated(text, schoolName, allSchools, MASCOT_LOOKUP)) {
                return settle(false, 'cross_contaminated_with_rival_school_or_mascot');
            }
    ...
    req.on('timeout', () => { req.destroy(); settle(false, 'timeout'); });
    req.end();
  });
}
...
async function run() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    process.stdout.write(`    [${healthy + broken + 1}/${total}] Auditing ${record.university}... `);
...
function targetedScrub() {
    const overlapping = [
        'Florida', 'Florida State', 'North Florida', 'South Florida', 'Florida A&M', 'Florida Atlantic', 'FIU',
        'Alabama', 'Alabama State', 'Alabama A&M', 'South Alabama',
        'Arkansas', 'Arkansas State', 'Arkansas-Pine Bluff', 'Central Arkansas',
        'Kansas', 'Kansas State',
        'Mississippi State', 'Ole Miss', 'Southern Miss', 'Mississippi Valley State',
        'Texas', 'Texas A&M', 'Texas State', 'Texas Southern', 'North Texas', 'UT Arlington', 'UTSA', 'UT Rio Grande Valley',
        'North Carolina', 'South Carolina', 'Coastal Carolina', 'Western Carolina', 'East Carolina',
        'Tennessee', 'Tennessee Tech', 'Middle Tennessee', 'UT Martin',
        'Georgia', 'Georgia Tech', 'Georgia Southern', 'Georgia State',
        'Louisiana', 'Louisiana Tech', 'LSU', 'ULM', 'New Orleans', 'Tulane',
        'State', 'Saint', 'State' // Generic keywords
    ];
    let count = 0;
    for (const record of data) {
        if (overlapping.some(o => record.university === o)) {
            console.log(`Force resetting ${record.university} for advanced re-audit...`);
            record.isChecked = false;
