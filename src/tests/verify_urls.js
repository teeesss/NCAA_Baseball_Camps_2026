...
async function verifyUrl(urlStr) {
    ...
    return new Promise((resolve) => {
        ...
        req.on('error', () => {
            ...
            const getReq = lib.request(parsed, {
                method: 'GET',
                timeout: 5000,
                headers: {
    ...
    try {
        ...
        if (results && results.length > 0) {
            ...
            for(let r of results) {
                if(r.link && (r.link.includes('athletics') || r.link.includes('sports') || r.link.endsWith('.edu/') || r.link.includes('ryzer'))) {
...
async function processData() {
    ...
    for (let i = 0; i < campsData.length; i += concurrency) {
        ...
        const tasks = chunk.map(async (school) => {
            ...
            school.whatToBring = "Glove, bat, helmet, cleats, baseball pants, water bottle, sunscreen.";

            let isMissing = !school.campUrl || school.campUrl === 'TBA' || school.campUrl.trim() === '' || school.campUrl === 'https://www.ncsasports.org/college-search?query=undefined';
            let urlWorks = false;
        ...
        console.log(`Processed ${Math.min(i + concurrency, campsData.length)} / ${campsData.length} records...`);
    }
