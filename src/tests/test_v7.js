...
function extractDataFromText(fullText) {
  ...
  for (let j = 0; j < lines.length; j++) {
    ...
    const hasCampKeyword =
      /camp|clinic|prospect|showcase|register|enroll|sign.?up|instruction/i.test(
        block,
      );
    const hasBaseballKeyword =
      /baseball|pitcher|batter|hitting|infield|outfield/i.test(block);
...
const TEST_SCHOOLS = [
  {
    ...
    campUrl: "https://www.baseballcampsmsu.com/",
    mascot: "Spartans",
  },
  {
    university: "Virginia",
    campUrl: "https://virginiasports.com/sports/baseball",
    mascot: "Cavaliers",
  },
  {
    university: "Alabama",
...
(async () => {
  ...
  for (const school of TEST_SCHOOLS) {
    ...
    try {
      const resp = await p.goto(school.campUrl, {
        ...
        els
          ...
          .filter(
            (l) =>
              ...
              /camp|register|clinic|prospect/.test(l.href + l.text),
          ),
      );
      ...
      tiers
        .slice(0, 5)
        .forEach((t) =>
          console.log(`  ✓ [${t.cost}] "${t.name}" — ${t.dates}`),
        );
    } catch (e) {
