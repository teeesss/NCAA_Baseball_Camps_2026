...
const DATA_FILE = path => require('path').resolve(__dirname, '../../camps_data.json');
...
d.forEach(school => {
    ...
    if (uniqueSchools.has(school.university)) {
        console.error(`❌ [DUPLICATE] Duplicate record found for ${school.university}`);
        errors++;
    }
    ...
    if (school.dates && school.dates !== 'TBA') {
        ...
        if (dateArray.length > 12) {
            console.error(`❌ [OVERFLOW] ${school.university} has ${dateArray.length} dates! (Garbage blob)`);
            missingQueue.add(school.university);
            errors++;
        }
        
        // Nested TBA check
        if (dateArray.some(d => d.toUpperCase().includes('TBA') && dateArray.length > 1)) {
            console.error(`❌ [GARBAGE] ${school.university} has 'TBA' mixed with other dates.`);
            errors++;
        }
    }
    ...
    if (school.contact && school.contact.includes('@')) {
        ...
        if (emailLower.includes('.edu')) {
            const domainStart = emailLower.substring(emailLower.indexOf('@') + 1, emailLower.lastIndexOf('.'));
    ...
    // 4. BAD LINKS
    const BLACKLISTED_AGGREGATORS = ['activekids.com', 'collegescoutingbureau.net', 'ussportscamps.com', 'ncsasports.org', 'captainu.com', 'berecruited.com', 'perfectgame.org'];
    if (school.campUrl && BLACKLISTED_AGGREGATORS.some(b => school.campUrl.toLowerCase().includes(b))) {
        console.error(`❌ [BLACKLIST] ${school.university} is pointing to a blacklisted aggregator: ${school.campUrl}`);
        missingQueue.add(school.university);
        errors++;
    }
...
if (missingQueue.size > 0) {
    fs.writeFileSync('missing_investigate_queue.txt', Array.from(missingQueue).join('\n'));
