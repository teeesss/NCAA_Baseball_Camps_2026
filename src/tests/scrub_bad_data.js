...
for (const record of data) {
  ...
  if (!needsScrub && record.dates && record.dates !== 'TBA') {
    ...
    if (placeholderPatterns.some(p => p.test(record.dates))) {
        const anySuspicious = dates.some(d => placeholderPatterns.some(p => p.test(d)));
        if (anySuspicious) {
            needsScrub = true;
  ...
  const forceReset = ['NJIT', 'Mississippi State', 'South Dakota State', 'University of Memphis', 'Memphis'];
  if (!needsScrub && forceReset.some(f => record.university.includes(f))) {
    // If they have suspiciously generic info, reset them
    if (record.cost === '$55' || record.cost === '$1' || (record.dates && record.dates.includes('2026-06-20'))) {
        needsScrub = true;
  ...
  if (needsScrub) {
    ...
    record.details = '(Scrubbed due to invalid data; queueing for re-extraction)';
    modified++;
  }
}
