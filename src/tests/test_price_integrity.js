...
data.forEach((school, schoolIdx) => {
  ...
  if (school.campTiers && Array.isArray(school.campTiers)) {
    school.campTiers.forEach((tier, tierIdx) => {
      ...
      const prices = (tier.cost.match(/\$[\d,]+(?:\.\d{2})?/g) || []).map((p) =>
      ...
      if (minPrice >= 0 && minPrice < PRICE_THRESHOLDS.VERIFY_MANUALLY) {
        let level = minPrice < PRICE_THRESHOLDS.CRITICAL_ANOMALY ? "CRITICAL"
                  : minPrice < PRICE_THRESHOLDS.SUSPICIOUS_LOW ? "SUSPICIOUS"
                  : "VERIFY";
  ...
  if (school.cost && school.cost !== "TBA" && school.cost.trim() !== "") {
    const prices = (school.cost.match(/\$[\d,]+(?:\.\d{2})?/g) || []).map((p) =>
      parseFloat(p.replace(/[\$,]/g, "")),
    );
    if (prices.length > 0) {
      const minPrice = Math.min(...prices.filter((p) => !isNaN(p)));
  ...
  if (fixMode) {
    ...
    violations.forEach((v) => {
      ...
      if (isCriticalAnomaly) {
        ...
        data[v.schoolIdx].details = "Data purged due to critical anomaly (found $1 pricing artifact). Re-queueing.";
    ...
    if (reextractSchools.size > 0) {
      ...
      for (const school of reextractSchools) {
        ...
        try {
          execSync(`node smart_extract.js --school="${school}"`, { stdio: "inherit" });
        } catch(e) { 
