...
function scoreUrl(url, school) {
    ...
    const bad = ['wikipedia', 'espn', 'facebook', 'twitter', 'instagram', 'fanfare', 'fandom', 'warrennolan', 'newsbreak'];
...
function getUniversityAliases(name) {
    ...
    let clean = name.replace(/University of | University| State University|College of | College/g, '').trim().toLowerCase();
...
function parseTiersMock(fullText) {
    ...
    for (let j = 0; j < lines.length; j++) {
        let line = lines[j];
        if (/(?:jun|jul|aug|june|july|august)\w*\s+\d/i.test(line) || /\b0?[6-8]\/\d{1,2}/i.test(line)) {
            let block = lines.slice(Math.max(0, j-2), Math.min(lines.length, j+5)).join(' | ');
            let nameMatch = block.match(/([A-Z0-9][A-Za-z0-9\s\/&]+(?:Camp|Clinic|Prospect|Showcase|Elite|Program|Session))/);
...
const url1 = 'https://vmikeydets.com/news/2025/5/28/sam-roberts-baseball-camps-kick-off-next-week.aspx';
...
assert(aliases.includes('arkansas-pine bluff'), `Expected 'arkansas-pine bluff' in ${JSON.stringify(aliases)}`);
