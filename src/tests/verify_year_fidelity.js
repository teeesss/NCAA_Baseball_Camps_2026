...
async function verifyFidelity() {
    ...
    let modifiedCount = 0;
    const targets = data.filter(d => d.dates !== 'TBA' || (d.campTiers && d.campTiers.length > 0));
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        if (!hasExplicit2026 && item.campUrl && item.campUrl.startsWith('http')) {
            try {
                ...
                await page.goto(item.campUrl, { waitUntil: 'networkidle2', timeout: 30000 });
        ...
        // 3. Enforcement
        if (!hasExplicit2026) {
            console.log(`  ‼️ REJECTED: ${item.university} has no verified 2026 data. Reverting to TBA.`);
            item.dates = 'TBA';
            item.cost = 'TBA';
            item.isVerified = false;
            item.isChecked = false; // allow re-extraction
            item.details = '(Rejected by Fidelity Audit: No 2026 mention found on page)';
            modifiedCount++;
        }
    }
