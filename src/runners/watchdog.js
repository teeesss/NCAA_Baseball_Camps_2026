...
const LOG_FILE = path.join(__dirname, "../../data/logs/smart_extract.log");
const SCRIPT = "smart_extract.js";
const INACTIVITY_TIME = 60000; // 60 seconds (Aggressive watchdog cutoff)
...
function runScript() {
  ...
  child.on("close", (code) => {
    if (code === 88) {
      ...
      logToWatchdog(
        `Script exited/killed (Code: ${code}). Restarting in 5s...`,
      );
      child = null;
  ...
  if (child && child.pid) {
    try {
      ...
      if (diff > INACTIVITY_TIME) {
        logToWatchdog(
          `! INACTIVITY DETECTED ! Log file hasn't changed for ${Math.round(diff / 1000)}s.`,
        );
        logToWatchdog(
          `Killing process tree for PID ${child.pid} and restarting...`,
        );
