...
function runInjection() {
  ...
  data.forEach((school) => {
    ...
    const isGatorEmail =
      school.email &&
      (school.email.toLowerCase().includes("ufl.edu") ||
        school.email.toLowerCase().includes("gators"));
    const isFlorida = school.university === "Florida";

    if (isGatorEmail && !isFlorida) {
      console.log(
        `  🛡️  Scrubbing contaminated Gator email from [${school.university}]: ${school.email}`,
      );
    ...
    if (
      school.details &&
      school.details.includes("Scrubbed due to invalid data")
    ) {
