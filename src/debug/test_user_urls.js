...
const targets = [
  ...
  { name: 'Cal State Fullerton', url: 'https://info.collegebaseballcamps.com/csf-baseball/' },
  { name: 'Central Michigan', url: 'https://cmubaseballcamps.totalcamps.com/About%20Us' }
];
...
async function check() {
  ...
  for (const target of targets) {
    ...
    try {
      ...
      const hasCamps = !content.toLowerCase().includes('no camps currently available');
  ...
  await browser.close();
  fs.writeFileSync('src/debug/results_user_targets.json', JSON.stringify(results, null, 2));
