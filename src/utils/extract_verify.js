...
function checkContamination(title, text, targetUni) {
    ...
    for (let other of allSchoolNames) {
        ...
        if (titleLower.includes(other.toLowerCase()) && !targetLower.includes(other.toLowerCase())) return other;
    }
    return null;
}
...
const run = async () => {
    ...
    const toProcess = data.filter(d => (!d.isChecked || (d.scriptVersion || 0) < 5) && !d.isVerified);
    
    console.log(`Starting V5 Ultra-Fidelity Stabilization. Target: ${toProcess.length} schools.`);

    for (let i = 0; i < toProcess.length; i++) {
        let camp = toProcess[i];
        console.log(`\n════════════════════════════════════════════════════════════`);
        console.log(`[${i + 1}/${toProcess.length}] Processing: ${camp.university}`);
        console.log(`════════════════════════════════════════════════════════════`);
        ...
        try {
            ...
            for (let q of queries) {
                try {
                    ...
                    await p.goto(`https://duckduckgo.com/html/?q=${encodeURIComponent(q)}`, { waitUntil: 'domcontentloaded', timeout: 10000 });
                    ...
                    if (ddgLinks && ddgLinks.length > 3) {
                        ...
                        await p.goto(`https://www.bing.com/search?q=${encodeURIComponent(q)}`, { waitUntil: 'domcontentloaded', timeout: 10000 });
                        let bingLinks = await p.evaluate(() => {
                            const found = Array.from(document.querySelectorAll('#b_results .b_algo h2 a, #b_results .b_algo a[href^="http"]')).map(a => a.href);
                            return found.filter(l => l && l.startsWith('http') && !l.includes('bing.com'));
                        });
                        if (bingLinks && bingLinks.length > 2) {
                            ...
                            await p.goto(`https://www.ask.com/web?q=${encodeURIComponent(q)}`, { waitUntil: 'domcontentloaded', timeout: 10000 });
                            ...
                            if (askLinks && askLinks.length > 0) {
                                ...
                                await p.goto(`https://search.yahoo.com/search?p=${encodeURIComponent(q)}`, { waitUntil: 'domcontentloaded', timeout: 10000 });
                                let yahooLinks = await p.evaluate(() => Array.from(document.querySelectorAll('.title a')).map(a => a.href));
            ...
            let guessed = [];
            if (cleanUni.length < 25) { // Avoid absurdly long invalid domains
            ...
            scored.slice(0, 5).forEach(s => console.log(`      ${s.score >= 50 ? "★" : "•"} [${s.score.toString().padStart(3)}] ${s.url}`));
            ...
            for (let item of scored) {
                ...
                try {
                    ...
                    if (!resp || resp.status() >= 400) { console.log(`      ✕ Unreachable/Error.`); continue; }
                    await delay(1500);
                    let text = await p.evaluate(() => document.body.innerText);
                    let title = await p.title();
                    
                    if (!aliases.some(a => text.toLowerCase().includes(a) || title.toLowerCase().includes(a) || candidate.includes(a.replace(/\s+/g, '')))) {
                        console.log(`      ✕ School name mismatch. Skip.`); continue;
                    }
                    if (checkContamination(title, text, camp.university)) {
                        console.log(`      ✕ Contamination check failed. Skip.`); continue;
                    }
                    ...
                    if (campTiers.length > 0) {
                        console.log(`   -> 🎯 SUCCESS — Progress saved. Found ${campTiers.length} tiers.`);
            ...
            if (!success) console.log(`   -> ❌ No 2026 dates found at any candidate.`);
            fs.writeFileSync('camps_data.json', JSON.stringify(data, null, 2));
