...
async function run() {
  ...
  const data = await fetchJson('http://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/teams?limit=400');
  ...
  for (const school of db) {
    ...
    if (!bestMatch) {
      ...
      if (hc) {
        bestMatch = espnTeams.find(et => normalize(et.location) === normalize(hc) || normalize(et.displayName) === normalize(hc));
      }
    }
    ...
    if (bestMatch && bestMatch.logos && bestMatch.logos.length > 0) {
      ...
      console.log(`  🎯 Matched [${school.university}] => ESPN [${bestMatch.displayName}]`);
      matched++;
    } else {
