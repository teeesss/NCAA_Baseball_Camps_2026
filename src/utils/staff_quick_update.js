...
if (!input) {
  console.log(
    'Usage: node src/utils/staff_quick_update.js "University | Head Coach | Camp POC | Email"',
  );
  console.log(
    'Example: node src/utils/staff_quick_update.js "SEC|Alabama|Coach X|POC Y|email@ua.edu"',
  );
  process.exit(1);
}
...
lines.forEach((line) => {
  ...
  const school = data.find(
    (s) => s.university.toLowerCase() === uni.toLowerCase(),
  );
  ...
  if (school) {
    ...
    console.log(
      `✅ [${school.university}] Updated: Coach=${school.headCoach}, POC=${school.campPOC}, Email=${school.email}`,
    );
  } else {
