...
const verifiedRecords = fs.existsSync(VERIFIED_FILE)
  ? JSON.parse(fs.readFileSync(VERIFIED_FILE, "utf8"))
  : [];
...
data.forEach((school) => {
  ...
  if (secSchools.includes(school.university)) {
    ...
    const wasVerified = verifiedRecords.find(
      (v) => v.university === school.university,
    );
    if (wasVerified) {
