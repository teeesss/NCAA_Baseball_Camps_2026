...
const backup = DATA_FILE.replace(
  ".json",
  `_backup_pricefix_${Date.now()}.json`,
);
...
const PURGE_NO_DATA = [
  {
    ...
    reason:
      "All pages (index, high-school-showcase, register) show 2025 data only. No 2026 content.",
    previousUrl: "https://www.texassouthernbaseballcamps.com/index.cfm",
  },
  {
    school: "Western Michigan University",
    reason:
      "wmubaseballcamps.com: 'No events'. wmuevents.com: no baseball content.",
...
for (const item of PURGE_NO_DATA) {
  ...
  if (!record && item.school.toLowerCase().includes("texas southern")) {
    record = data.find((d) =>
      TX_SOUTHERN_ALIASES.some((a) => d.university.toLowerCase() === a),
    );
  }
  if (!record) {
...
const sru = data.find((d) =>
  d.university.toLowerCase().includes("slippery rock"),
);
if (sru) {
  console.log(`🗑️  PURGE (3rd-party showcase): ${sru.university}`);
  console.log(
    `   ryzer.com event is "Messer Baseball LLC Unsigned Showcase" — not SRU's own camp.`,
  );
  console.log(
    `   Engine extracted only the fee breakdown ($15/$19/$20), missing real prices ($215/$294).`,
  );
  sru.campTiers = [];
  sru.dates = "TBA";
  sru.cost = "TBA";
  ...
  sru.sourceUrl = null;
  sru.auditStatus =
    "THIRD_PARTY_SHOWCASE — Messer Baseball LLC event at SRU, not university-run camp";
  sru.isChecked = false;
  sru.scriptVersion = 0;
...
const gonzaga = data.find((d) =>
  d.university.toLowerCase().includes("gonzaga"),
);
if (gonzaga) {
  console.log(`✏️  FIX: ${gonzaga.university}`);
  console.log(
    `   Verified via browser: zagsbaseballcamps.com/shop/EVENT is real.`,
  );
  console.log(
    `   $50 = "Pups Camp" ages 4-6. Legitimate youth tier. No HS prospect camp posted yet.`,
  );
  gonzaga.campTiers = [
    {
      name: "Helena Future Zags Camp",
    ...
    {
      name: "Lil Zags Camp #1",
      dates: "July 1-2, 2026",
      cost: "$125",
      ageGroup: "Ages 6-10",
    },
    {
      name: "Big Dogs Camp #1",
      dates: "July 7-8, 2026",
      cost: "$140",
      ageGroup: "Ages 10-14",
    },
    ...
    {
      name: "Lil Zags Camp #2",
      dates: "July 14-15, 2026",
      cost: "$125",
      ageGroup: "Ages 6-10",
    },
    {
      name: "Big Dogs Camp #2",
      dates: "July 14-15, 2026",
      cost: "$140",
      ageGroup: "Ages 10-14",
    },
    {
      name: "Advanced Skills Camp",
      dates: "July 21-22, 2026",
      cost: "$140",
      ageGroup: "Ages 10-14",
    },
    {
      name: "Midsummer Classic 18U Team",
      dates: "June 25-28 / July 16-19, 2026",
      cost: "$1,500",
      ageGroup: "18U Teams — SOLD OUT",
    },
    {
      name: "Midsummer Classic 16U Team",
      dates: "July 9-12, 2026",
      cost: "$1,500",
      ageGroup: "16U Teams — SOLD OUT",
    },
  ];
  gonzaga.dates =
    "June 22-23, July 1-2, July 7-8, July 14-15, July 21-22, 2026";
  ...
  gonzaga.auditStatus =
    "VERIFIED_BROWSER_20260407 — Youth camps confirmed. No HS prospect camp posted yet.";
...
if (swosu) {
  ...
  swosu.campTiers = [
    {
      name: "Junior High Camp (Prospect Prep)",
      dates: "May 25-27, 2026",
      cost: "$122",
      ageGroup: "Grades 7-8 (Ages 13-15)",
    },
    {
      name: "Fundamentals Camp",
      dates: "June 1-4, 2026",
      cost: "$138",
      ageGroup: "Ages 6-12",
    },
  ...
  swosu.sourceUrl =
    "https://www.zacksaundersbaseballcamps.com/hitting-camp.cfm";
  swosu.auditStatus =
    "VERIFIED_BROWSER_20260407 — All 4 camp pages confirmed real 2026 data";
...
const lcu = data.find((d) =>
  d.university.toLowerCase().includes("lubbock christian"),
);
if (lcu) {
  ...
  lcu.auditStatus =
    "PRICE_CHECK_NEEDED — lcubaseballcamps.com is youth-only (Grades 2-6). No HS camp confirmed.";
  lcu.cost = "TBA"; // Clear suspicious price until HS camp is confirmed
  if (lcu.campTiers) {
    lcu.campTiers.forEach((t) => {
      if (t.cost === "$59") t.cost = "TBA";
    });
  }
