...
function checkContamination(title, text, targetUni) {
    ...
    for (let other of allSchoolNames) {
        ...
        if (regex.test(titleLower)) {
            if (!targetLower.includes(otherLower) && !titleLower.includes(targetLower)) return other;
        }
    }
    return null;
}
...
function scoreUrl(url, school, isGuessed = false) {
    ...
    let mascot = (school.mascot || getMascot(school.university) || '').toLowerCase();
    ...
    let shortS = s.replace(/university|state|college/gi, '').trim().replace(/\s+/g, '');
    ...
    const bad = ['wikipedia', 'espn', 'facebook', 'twitter', 'instagram', 'fandom', 'warrennolan', 'newsbreak', 'ussportscamps', 'zhihu', 'reddit', 'yelp', 'tripadvisor', 'search.yahoo.com', 'images.search.yahoo.com', 'video.search.yahoo.com', 'bing.com/images', 'bing.com/videos'];
...
function extractDataFromText(fullText) {
    ...
    for (let j = 0; j < lines.length; j++) {
        ...
        if (/(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\s+\d{1,2}/i.test(line) || /\b(0?[1-9]|1[0-2])\/\d{1,2}/.test(line)) {
            let block = lines.slice(Math.max(0, j-3), Math.min(lines.length, j+6)).join(' | ');
            let low = block.toLowerCase();
            
            // --- STRICT FILTERS ---
            // 1. Skip schedule-like lines
            if (low.includes(' vs ') || low.includes(' vs. ') || low.includes(' @ ') || low.includes(' vs ') || /\d{1,2}:\d{2}\s*(?:am|pm)/i.test(block)) continue;
            ...
            if (low.includes('basketball') || low.includes('soccer') || low.includes('softball') || low.includes('volleyball')) continue;

            // 4. Require explicit camp terminology in the immediate block
            const campKeywords = ['camp', 'clinic', 'prospect', 'showcase', 'elite', 'program', 'instruction', 'lesson'];
            ...
            let dateMatch = block.match(/(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\s+\d{1,2}(?:\w{2})?)|(?:\b(0?[1-9]|1[0-2])\/\d{1,2})/gi);
    ...
    const filtered = campTiers.filter((v, i, a) => a.findIndex(t => t.name === v.name && t.dates === v.dates) === i);
    if (filtered.length > 0) {
        log(`       📋 [Tiers] Found ${filtered.length} entries. Preview: ${filtered.slice(0, 2).map(t => t.name).join(' | ')}...`);
    } else {
...
const run = async () => {
    ...
    const toProcess = data.filter(d => (!d.isChecked || (d.scriptVersion || 0) < 6) && !d.isVerified);
    
    log(`Starting V6 Ultra-Fidelity Stabilization. Target: ${toProcess.length} schools.`);
    ...
    for (let i = 0; i < toProcess.length; i++) {
        ...
        await p.setViewport({ width: 1920 + Math.floor(Math.random() * 100), height: 1080 + Math.floor(Math.random() * 100) });
        ...
        const humanMove = async (page) => {
            const target = { x: 300 + Math.random() * 800, y: 300 + Math.random() * 600 };
        ...
        try {
            ...
            let cleanQuery = camp.university.replace(/university|state|college/gi, '').trim();
            ...
            // Clean links
            searchLinks = [...new Set(searchLinks)].filter(l => l && l.startsWith('http'));
            ...
            scored.slice(0, 3).forEach(s => log(`      ${s.score >= 50 ? "★" : "•"} [${s.score.toString().padStart(3)}] ${s.url}`));
            ...
            for (let item of scored) {
                ...
                try {
                    ...
                    if (!aliases.some(a => text.toLowerCase().includes(a) || title.toLowerCase().includes(a) || candidate.toLowerCase().includes(a.replace(/\s+/g, '')))) {
                        log(`      ✕ School name mismatch.`); continue;
                    }

                    // CONTAMINATION: Is this an opponent's schedule page?
                    const culprit = checkContamination(title, text, camp.university);
                    if (culprit) {
                        log(`      ✕ Contamination suspected (${culprit}). Skip.`); continue;
                    }
                    ...
                    let filteredSub = subLinks.filter(l => {
                        ...
                        const bad = ['wikipedia', 'espn', 'facebook', 'twitter', 'instagram', 'fandom', 'warrennolan', 'newsbreak', 'ussportscamps', 'zhihu', 'reddit', 'yelp', 'tripadvisor'];
                        if (bad.some(b => hl.includes(b))) return false;

                        // Only crawl internal or camp-specific external links
                        if (hl.includes('camp') || hl.includes('register') || hl.includes('detail') || hl.includes('prospect')) return true;
                    ...
                    for (let sl of filteredSub) {
                        try {
                            ...
                            if (st && st.toLowerCase().includes('baseball')) fullText += '\n' + st;
                            await sp.close();
                        } catch(e) {}
                    }
