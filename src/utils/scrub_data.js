...
function run() {
    ...
    data.forEach(record => {
        ...
        if (!record.lastChecked) { record.lastChecked = now; schemaInitCount++; }
        ...
        const detailsHasNoStatusMsg = record.details && record.details.toLowerCase().includes('no 2026 camps');
        ...
        if (record.email && record.campPOC) {
            ...
            if (emailClean === pocClean) {
                ...
                if (!record.updateLog.includes(logMsg)) record.updateLog.unshift(logMsg);
            }
        }
