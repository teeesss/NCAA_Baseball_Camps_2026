...
const REJECT_SPORTS = [
    ...
    'golf', 'volleyball', 'wrestling', 'lacrosse', 'softball', 
    'hockey', 'track and field'
];

const OFFICIAL_PLATFORMS = [
    'ryzer.com', 'thegoodgame.com', 'totalcamps.com', 'eventlink.com',
    'campsnetwork.com', 'abcsportscamps.com', 'playnsports.com', 'reflow.com',
    'active.com', 'activekids.com'
];
...
const GENERIC_EMAIL_PREFIXES = ['example', 'noreply', 'sentry', 'webmaster', 'support'];
const SUBDOMAIN_BLACKLIST_PREFIXES = ['m.', 'mobile.', 'beta.', 'dev.', 'stage.'];

// ── Shared Logic Helpers ─────────────────────────────────────
function isBlacklistedUrl(url) {
    if (!url) return true;
    const lower = url.toLowerCase();
    return BLACKLISTED_DOMAINS.some(domain => lower.includes(domain.toLowerCase()));
}

function isWrongSport(text) {
    if (!text) return false;
    const lower = text.toLowerCase();
    if (lower.includes('baseball') || lower.includes('pitcher') || lower.includes('hitting')) return false;
