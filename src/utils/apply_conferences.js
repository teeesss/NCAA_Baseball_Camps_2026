...
const authConfPath = path.join(__dirname, '../../raw_authoritative_conferences.json');
...
camps.forEach(school => {
    ...
    if (!newConf && lowerName === 'tennessee-martin') newConf = 'Ohio Valley Conference';
    if (!newConf && lowerName === 'south carolina upstate') newConf = 'Big South Conference';
    if (!newConf && lowerName === 'west georgia') newConf = 'Atlantic Sun Conference'; // Moved to D1 ASUN
    ...
    if (newConf) {
        ...
        if (school.conference !== finale) {
            console.log(`Updating ${school.university}: ${school.conference} -> ${finale}`);
            school.conference = finale;
            fixed++;
        }
    } else {
        if (school.division === 'DI') {
            console.log(`⚠️ Unmatched DI: ${school.university} (current: ${school.conference})`);
            missing++;
        }
