...
if (args.length === 0) {
  console.log(
    'Usage: node run_batch.js --teams="School A,School B" OR --conference="SEC"',
  );
  process.exit(1);
}
...
if (teamsArg) {
  ...
  targetSchools = data.filter((r) =>
    teams.some((t) => r.university.toLowerCase().includes(t)),
  );
} else if (confArg) {
  const conf = confArg.split("=")[1].trim().toLowerCase();
  targetSchools = data.filter(
    (r) => r.conference && r.conference.toLowerCase() === conf,
  );
}
...
console.log(
  "✨ Batch processing complete! Please review results then run `npm run generate:html` and `npm run deploy`.",
);
