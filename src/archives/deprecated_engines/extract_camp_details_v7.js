...
function getCoachName(camp) {
  ...
  let raw = camp.contact
    ...
    .replace(/\(.*?\)/g, "")
    .trim();
  if (raw.toLowerCase().includes("tba") || raw.includes("@") || raw.length < 3)
    return "";
  return raw;
}
...
function extractDataFromText(fullText) {
  ...
  const COST_PATTERN =
    /(?:\$[\d,]+(?:\.\d{2})?(?:\s*[-–]\s*\$[\d,]+(?:\.\d{2})?)?|FREE|Complimentary|No cost)/i;
  ...
  for (const q of queries.slice(0, 3)) {
    for (const provider of providers) {
      ...
      try {
        ...
        await page.goto(provider.url(q), {
...
const run = async () => {
  ...
  const toProcess = data.filter(
    (d) => (!d.isChecked || (d.scriptVersion || 0) < 7) && !d.isVerified,
  );
  log(
