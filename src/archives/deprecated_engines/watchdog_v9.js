...
function startScript() {
  console.log(
    `[WATCHDOG-V9] [${new Date().toLocaleTimeString()}] Starting V9 extraction (Attempt #1)...`,
  );
  subProcess = spawn("node", [SCRIPT], { stdio: "inherit", shell: true });

  subProcess.on("exit", (code) => {
    console.log(
      `[WATCHDOG-V9] Script exited with code ${code}. Restarting in 5s...`,
    );
...
function checkActivity() {
  try {
    ...
    if (size > lastSize) {
      ...
      if (idle > TIMEOUT) {
        console.log(
          `[WATCHDOG-V9] [${new Date().toLocaleTimeString()}] 🚨 HANG DETECTED (${Math.floor(idle / 1000)}s inactive). Killing and restarting...`,
        );
        if (subProcess) {
          ...
          exec(`taskkill /F /T /PID ${subProcess.pid}`, () => {
            console.log(
              `[WATCHDOG-V9] Process tree ${subProcess.pid} terminated.`,
            );
            startScript();
          });
        } else {
