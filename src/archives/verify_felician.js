...
const run = async () => {
  ...
  if (!camp) {
    console.log("Felician not found");
    process.exit(0);
  }
  ...
  try {
    ...
    for (let q of queries) {
      ...
      let braveLinks = await p.evaluate(() =>
        Array.from(document.querySelectorAll("a.result-header")).map(
          (a) => a.href,
        ),
      );
    ...
    let scored = searchLinks
      .map((u) => ({ url: u, score: scoreUrl(u, camp, false) }))
