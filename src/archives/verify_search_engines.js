...
async function testEngines() {
  ...
  const engines = [
    ...
    },
    {
      ...
      selector: "a.result-header, h3 a, #results a",
    },
    {
      name: "Yahoo",
      url: `https://search.yahoo.com/search?p=${encodeURIComponent(query)}`,
      selector: ".title a, .compTitle a, #web h3 a, .algo a",
    },
    {
      name: "Ask",
      url: `https://www.ask.com/web?q=${encodeURIComponent(query)}`,
      selector: ".PartialSearchResults-item-title-link, a.result-link, h3 a",
    },
    {
      name: "Google",
      url: `https://www.google.com/search?q=${encodeURIComponent(query)}`,
      selector: '#search a[href^="http"]:not([href*="google.com"])',
    },
  ];
  ...
  for (const e of engines) {
    try {
      ...
      if (count === 0) {
        console.log(
          `⚠️  Warning: 0 results for ${e.name}. Selector might need update.`,
        );
      }
    } catch (err) {
