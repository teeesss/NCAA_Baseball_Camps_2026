...
async function test() {
  ...
  for (let q of queries) {
    ...
    let ddgLinks = await p.evaluate(() =>
      Array.from(document.querySelectorAll(".result__a")).map((a) => a.href),
    );
    ...
    if (ddgLinks && ddgLinks.length > 3) {
      ...
      await p.goto(`https://www.bing.com/search?q=${encodeURIComponent(q)}`, {
        waitUntil: "domcontentloaded",
      });
      let bingLinks = await p.evaluate(() =>
        Array.from(document.querySelectorAll("#b_results .b_algo h2 a")).map(
          (a) => a.href,
        ),
      );
      if (bingLinks && bingLinks.length > 0) {
        ...
        let askLinks = await p.evaluate(() =>
          Array.from(
            document.querySelectorAll(".PartialSearchResults-item-title-link"),
          ).map((a) => a.href),
        );
  ...
  scored.forEach((s) =>
    console.log(
      `[${s.score}] ${s.isGuessed ? "(GUESSED)" : "(SEARCH)"} ${s.url}`,
    ),
  );
