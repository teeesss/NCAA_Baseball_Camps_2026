...
function simulateFilter(item, currentDiv) {
  ...
  if (currentDiv === "DI" || currentDiv === "DII")
    return item.division === currentDiv;
...
const filterTests = [
  {
    desc: "Latest Updates: matches any update",
    item: { lastUpdateDate: "2026-04-04" },
    div: "updates",
    expected: true,
  },
  {
    desc: "Latest Updates: fails if no update",
    item: { lastUpdateDate: "" },
    div: "updates",
    expected: false,
  },
  {
    desc: "New Camp Dates: matches if date update exists",
    item: { datesUpdateDate: "2026-04-04" },
    div: "newdates",
    expected: true,
  },
  {
    desc: "New Camp Dates: fails if no date update",
    item: { datesUpdateDate: "" },
    div: "newdates",
    expected: false,
  },
  {
    desc: "New Camp Dates: only cares about timestamp",
    item: { datesUpdateDate: "2026-04-04", dates: "TBA" },
    div: "newdates",
    expected: true,
  },
];
...
try {
  ...
  if (
    data[0].details === "" &&
    data[0].datesUpdateDate &&
    data[0].lastUpdateDate
  ) {
    console.log(
      "✅ Arizona-style conflict fix correctly populated granular timestamps.",
    );
  } else {
  ...
  if (
    data[1].campPOC === "N/A" &&
    data[1].contactUpdateDate &&
    data[1].lastUpdateDate
  ) {
    console.log(
      "✅ Contact de-duplication fix correctly populated granular timestamps.",
    );
  } else {
