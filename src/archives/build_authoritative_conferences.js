...
async function run() {
  const d1 = await scrapeWiki(
    "https://en.wikipedia.org/wiki/List_of_NCAA_Division_I_baseball_programs",
  );
  const d2 = await scrapeWiki(
  ...
  console.log(
    `Saved ${Object.keys(finalMap).length} records to raw_authoritative_conferences.json`,
  );
}
