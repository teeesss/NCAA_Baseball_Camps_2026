...
async function run() {
    ...
    console.log(`Found ${toProcess.length} generic root domains. Fetching actual links for the first 30...`);
    ...
    for (let i = 0; i < toProcess.length; i++) {
        let d = toProcess[i];
        console.log(`[${i+1}/${toProcess.length}] Searching for: ${d.university} baseball camp`);
        try {
            const results = await googleIt({ query: d.university + ' baseball camp 2026 registration', disableConsole: true, limit: 3 });
