...
// Clear log
try { fs.writeFileSync(LOG_FILE, `=== V9 Extraction Started ${new Date().toISOString()} ===\n`); } catch(e) {}
...
function scoreUrl(url, school, isGuessed = false) {
    ...
    const shortS = s.replace(/university|state|college| of /gi, '').trim().replace(/\s+/g, '');
    if (shortS.length > 3 && u.includes(shortS)) score += 15;

    if (u.includes('ryzer.com') || u.includes('totalcamps.com') || u.includes('active.com')) score += 60;
    if (isGuessed) score -= 40;

    const bad = ['wikipedia', 'espn', 'facebook', 'twitter', 'instagram', 'fandom', 'warrennolan', 'surveygizmo.com'];
...
function extractDataFromText(fullText) {
    ...
    const SUMMER_NUMERIC     = '(?:0?[67]|0?8)\\/(?:0[1-9]|[12]\\d|3[01])(?:\\/(?:2026|26))?';
    const DATE_PATTERN = new RegExp(`(?:(?:${SUMMER_MONTH_NAMES})\\s+\\d{1,2}|${SUMMER_NUMERIC})`, 'gi');
    const COST_PATTERN = /(?:\$[\d,]+(?:\.\d{2})?(?:\s*[-–\/]\s*\$[\d,]+(?:\.\d{2})?)?(?:\s*\+)?|FREE|Complimentary)/i;
    ...
    for (let j = 0; j < lines.length; j++) {
        ...
        const block = lines.slice(Math.max(0, j - 2), Math.min(lines.length, j + 5)).join(' | ');
        const low = block.toLowerCase();

        if (low.includes(' vs ') || low.includes('box score') || low.includes('2024')) continue;
        if (!/camp|clinic|prospect|showcase|baseball/i.test(block)) continue;

        const dateMatches = [...block.matchAll(new RegExp(DATE_PATTERN.source, 'gi'))].map(m => m[0]);
        if (dateMatches.length === 0) continue;

        const costMatch = block.match(COST_PATTERN);
        campTiers.push({
            name:  (block.match(/([A-Z][A-Za-z\s\/&-]+(?:Camp|Clinic|Prospect|Elite))/)?.[1] || 'Baseball Camp').trim(),
...
function harvestEmails(text) {
    ...
    return [...new Set(matches.filter(e => !/bootstrap|example|domain/i.test(e)))];
}
...
async function runSearchQueries(page, camp, idx) {
    ...
    const q = mascot ? `${camp.university} ${mascot} baseball camps 2026` : `"${camp.university}" baseball camps 2026`;
    const links = [];
    try {
        await page.goto(engine.url(q), { waitUntil: 'domcontentloaded', timeout: 20000 });
        const found = await page.evaluate(sel => Array.from(document.querySelectorAll(sel)).map(a => a.href), engine.selector);
        links.push(...found.filter(h => h && h.startsWith('http') && !h.includes(engine.name.toLowerCase())));
    } catch(e) { log(`      ✕ [${engine.name}] Error: ${e.message.substring(0, 40)}`); }
    ...
    if (links.length < 5 && coach) {
        ...
        try {
            ...
            const found2 = await page.evaluate(sel => Array.from(document.querySelectorAll(sel)).map(a => a.href), engine.selector);
            links.push(...found2.filter(h => h && h.startsWith('http') && !h.includes(engine.name.toLowerCase())));
        } catch(e) {}
    }
...
// ── MAIN RUNNER ────────────────────────────────────────────────────────────────
const run = async () => {
    const toProcess = data.filter(d => (!d.isChecked || (d.scriptVersion || 0) < 9) && !d.isVerified);
    ...
    for (let i = 0; i < toProcess.length; i++) {
        ...
        try {
            ...
            const scored = searchLinks.map(u => ({ url: u, score: scoreUrl(u, camp) }))
            ...
            for (const item of scored) {
                ...
                try {
                    ...
                    // Validation
                    const matchedAlias = aliases.find(a => text.toLowerCase().includes(a) || title.toLowerCase().includes(a));
                    ...
                    // Gather sub-links
                    const subLinks = await p.$$eval('a', els => els.map(a => a.href).filter(h => h.startsWith('http')).slice(0, 6));
                    let fullText = text;
                    ...
                    for (const sl of subLinks) {
                        try {
                            ...
                            await sp.goto(sl, { waitUntil: 'domcontentloaded', timeout: 10000 });
