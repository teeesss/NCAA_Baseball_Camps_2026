  ...
  return new Promise((resolve) => {
    try {
      ...
      const req = lib.request(
        { method: 'HEAD', hostname: parsed.hostname, port: parsed.port || (parsed.protocol === 'https:' ? 443 : 80), path: parsed.pathname, timeout: 6000 },
...
async function run() {
  ...
  console.log(`🔍 Found ${missing.length} schools missing logos. Processing...\n`);
  ...
  for (const school of missing) {
    ...
    if (!domain && school.campUrl && school.campUrl.startsWith('http')) {
      try {
        ...
        if (host.endsWith('.edu') || /go[a-z]+\.com$/.test(host) || /athletics\.com$/.test(host) || /sports\.com$/.test(host)) {
          domain = host;
        }
      } catch (_) {}
    }
    ...
    if (!domain) {
      try {
        const searchName = uni
          .replace(/ University| College| State$/ig, '')
          .replace(/&/g, 'and')
          .trim();
        const res = await new Promise((resolve) => {
          http.get(`http://universities.hipolabs.com/search?name=${encodeURIComponent(searchName)}&country=United+States`, (r) => {
            let d = '';
            r.on('data', c => d += c);
        ...
        if (res && res.length > 0) {
          const best = res.find(r => r.name.toLowerCase().includes(uni.toLowerCase())) || res[0];
          if (best && best.domains && best.domains.length > 0) domain = best.domains[0];
        }
      } catch (_) {}
    }
    ...
    console.log(`  ✅ ${uni}: ${clearbitOk ? 'Clearbit' : 'Favicon'} → ${school.logoFile}`);
    fixed++;
