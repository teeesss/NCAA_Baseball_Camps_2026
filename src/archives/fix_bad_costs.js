...
const FIXES = [
  {
    university: "LSU",
    reason:
      'cost field is corrupted comma-only string (", , , ") — no actual price data',
    setCost: null,
  ...
  },
  {
    university: "Illinois",
    reason:
      'cost "$59.99" is below $100 floor and from stale 2015 athletics URL — not a real camp price',
    setCost: null,
  ...
  },
  {
    ...
    reason:
      'cost "$50" is below $100 floor; campUrl is a DuckDuckGo redirect to a FOOTBALL camp — wrong sport',
    setCost: null,
...
FIXES.forEach((fix) => {
  ...
  console.log(
    `   Before: cost=${JSON.stringify(record.cost)}, campUrl=${record.campUrl}`,
  );
  ...
  console.log(
    `   After:  cost=${JSON.stringify(record.cost)}, campUrl=${record.campUrl}`,
  );
  fixed++;
});
