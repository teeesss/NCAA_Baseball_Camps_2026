...
function extractData(text, url) {
  ...
  if (costs.length) {
    let validCosts = costs.filter(
      (c) => c >= COST_RANGE.MIN && c <= COST_RANGE.MAX,
    );
...
assert(
  isWrongSport("Welcome to Oklahoma State Football Camp 2026") === true,
  "Football camp page rejected",
);

assert(
  isWrongSport("Basketball Camp - Summer 2026 Clinic") === true,
  "Basketball clinic page rejected",
);

assert(
  isWrongSport(
    "Welcome to Oklahoma State Baseball Camp 2026 - we also have football camp",
  ) === false,
  "Mixed page WITH baseball keyword accepted",
);

assert(
  isWrongSport("Summer Camp 2026 - Youth Skills Training") === false,
  "Generic camp page without sport keyword accepted",
);

assert(
  isWrongSport("Volleyball Camp dates June 15-18") === true,
  "Volleyball camp rejected",
);

assert(
  isWrongSport("University Baseball Camp Registration - $250") === false,
...
assert(
  isTeamCampOrLegacy("2025 Summer Baseball Team Camp Dates") === true,
  "Legacy 2025 page rejected",
);

assert(
  isTeamCampOrLegacy("Team Camp Only - June 2026") === true,
  "Team-only camp rejected",
);

assert(
  isTeamCampOrLegacy("Team Camp & Individual Skills Camp - June 2026") ===
    false,
  "Team + Individual camp accepted",
);

assert(
  isTeamCampOrLegacy("2025 results and 2026 upcoming camp dates") === false,
  "Page mentioning both years accepted",
);

assert(
  isTeamCampOrLegacy("Individual Prospect Camp 2026 Dates") === false,
...
const dateTest1 = extractData(
  "June 15, 2026 - Summer Baseball Camp - $250",
  "https://example.edu/camp",
);
assert(
  dateTest1.dates !== null && dateTest1.dates.includes("June 15"),
  "June 2026 date extracted correctly",
);

const dateTest2 = extractData(
  "Camp scheduled for January 10, 2025",
  "https://example.edu/camp",
);
assert(dateTest2.dates === null, "2025 date NOT extracted (wrong year)");

const dateTest3 = extractData(
  "07/20/2026 Baseball Prospect Camp",
  "https://example.edu/camp",
);
assert(
  dateTest3.dates !== null && dateTest3.dates.includes("07/20/2026"),
  "Numeric 2026 date extracted",
);

const dateTest4 = extractData(
  "August 5-8, 2026 Youth Baseball Camp",
  "https://example.edu/camp",
);
assert(dateTest4.dates !== null, "August multi-day range extracted");

// ── TEST GROUP 4: Cost Filtering ──
console.log("\n📋 GROUP 4: Cost Filtering ($100-$1500)");

const costTest1 = extractData(
  "Baseball Camp 2026 - $250 per player - June 10, 2026",
  "https://example.edu",
);
assert(costTest1.costVal === 250, "$250 accepted (normal range)");

const costTest2 = extractData(
  "Baseball Camp 2026 - $50 discount - June 10, 2026",
  "https://example.edu",
);
assert(costTest2.costVal === null, "$50 rejected (below $100 floor)");

const costTest3 = extractData(
  "Baseball Camp 2026 - $2000 team package - June 10, 2026",
  "https://example.edu",
);
assert(costTest3.costVal === null, "$2000 rejected (above $1500 ceiling)");

const costTest4 = extractData(
  "Baseball Camp - $150 youth | $400 prospect | $800 elite - June 10, 2026",
  "https://example.edu",
);
assert(
  costTest4.costVal === 150,
  "Multiple costs: lowest valid ($150) selected",
);

const costTest5 = extractData(
  "Baseball Camp 2026 - $1200 Elite Prospect Camp - July 1, 2026",
  "https://example.edu",
);
assert(
  costTest5.costVal === 1200,
...
assert(
  emailTest1.email === "baseball@school.edu",
...
assert(
  emailTest2.email === "baseball@school.edu",
...
assert(
  emailTest3.email === "camps@university.edu",
...
assert(
  okState.dates === null && okState.cost === null,
...
assert(
  virginia.email === "baseball@virginia.edu",
