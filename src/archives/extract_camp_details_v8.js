...
try { fs.writeFileSync(LOG_FILE, `=== V8 Extraction Started ${new Date().toISOString()} ===\n`); } catch(e) {}
...
function scoreUrl(url, school, isGuessed = false) {
    ...
    if (u.includes('prestocamp') || u.includes('campsite') || u.includes('imleagues')) score += 30;
    ...
    if (u.includes('/news/') || u.includes('/article/') || u.includes('/release/')) score -= 20;
    ...
    const bad = ['wikipedia', 'espn', 'facebook', 'twitter', 'instagram', 'fandom',
                 ...
                 'thebaseballcube', 'ratemyprofessors', 'niche.com', 'collegeconfidential',
                 'surveygizmo.com'];
...
function extractDataFromText(fullText) {
    ...
    const SUMMER_NUMERIC     = '(?:0?[67]|0?8)\\/(?:0[1-9]|[12]\\d|3[01])(?:\\/(?:2026|26))?'; // 6/xx, 7/xx, 8/xx with optional /2026 or /26 ONLY
    ...
    const COST_PATTERN = /(?:\$[\d,]+(?:\.\d{2})?(?:\s*[-–\/]\s*\$[\d,]+(?:\.\d{2})?)?(?:\s*\+)?|FREE|Complimentary|No cost|Free of charge)/i;
...
async function runSearchQueries(page, camp, schoolIdx) {
    ...
    log(`   → [ENGINE] ${engine.name} (school slot ${schoolIdx % PROVIDERS.length + 1}/3)`);
...
const run = async () => {
    ...
    const toProcess = data.filter(d => (!d.isChecked || (d.scriptVersion || 0) < 8) && !d.isVerified);
    ...
    log(`   Filters: SUMMER ONLY (Jun/Jul/Aug) | Relaxed gating (baseball OR camp) | Coach-name queries validated`);
    ...
    for (let i = 0; i < toProcess.length; i++) {
        ...
        log(`   Coach: ${coachDisplay || '(none/TBA)'} | Contact raw: "${camp.contact || ''}"`);
        log(`${'═'.repeat(60)}`);
        ...
        try {
            ...
            .filter((x, idx, self) => self.findIndex(y => y.url === x.url) === idx) // dedupe
            .filter(x => x.score > -50)
            .slice(0, 18);
            ...
            for (const item of scored) {
                ...
                try {
                    ...
                    const campKeywords = ['camp', 'clinic', 'register', 'prospect', 'showcase', 'detail', 'event', 'elite', 'youth', 'summer'];
                    const blacklist    = ['wikipedia', 'facebook', 'twitter', 'instagram', 'youtube', 'espn', 'fandom', 'reddit', 'warrennolan', 'surveygizmo.com'];
                    ...
                    const filteredSub = subLinks.filter(l => {
                        ...
                        if (hl.includes('/news/') || hl.includes('/article/')) return false;
                    ...
                    for (const sl of filteredSub) {
                        ...
                        try {
                            ...
                            const hasCampContent     = /camp|clinic|register|prospect|summer/i.test(st);
                    ...
                    if (campTiers.length > 0) {
                        ...
                        success = true;
                        log(`   → 🎯 SUCCESS — ${campTiers.length} tiers. Cost: ${camp.cost || 'TBA'} | Dates: ${camp.dates.substring(0, 60)}`);
                        break;
                    }
                } catch(e) {
