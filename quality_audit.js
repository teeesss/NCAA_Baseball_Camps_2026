...
function hasRealDate(record) {
  const dateText = [record.dates || '', record.details || '', ...(record.campTiers || []).map(t => t.dates || '')].join(' ');
  const has2026 = DATE_2026_REGEX.test(dateText) || DATE_NUMERIC_REGEX.test(dateText);
  const isStale  = DATE_STALE_REGEX.test(dateText) && !has2026;
  return has2026 && !isStale;
}

function hasRealCost(record) {
  const costText = [record.cost || '', ...(record.campTiers || []).map(t => t.cost || '')].join(' ');
  return /\$\s*\d+/.test(costText);
}

function hasContact(record) {
  const c = (record.contact || '').trim();
  return c.length > 3 && !c.toLowerCase().includes('tbd') && !c.toLowerCase().includes('n/a');
}

function hasEmail(record) {
  return /@[a-zA-Z0-9.-]+\.[a-z]{2,}/.test(record.contact || '') ||
...
function scoreRecord(record) {
  ...
  if (!generic && dates && cost && contact && email) { score = 100; tier = 'COMPLETE'; }
  else if (!generic && dates && cost)                { score = 75;  tier = 'GOOD';     }
  else if (!generic && dates)                        { score = 50;  tier = 'PARTIAL';  }
  else if (!generic)                                 { score = 25;  tier = 'LOW';      }
  else                                               { score = 0;   tier = 'EMPTY';   }
...
function run() {
  ...
  if (!fs.existsSync(DATA_FILE)) {
    console.error('❌ camps_data.json not found!');
    process.exit(1);
  }
  ...
  const { CONFERENCE_LOOKUP } = require('./src/utils/conference_lookup.js');
  ...
  function getPrestige(university, division) {
    ...
    const elite = ['Arkansas', 'LSU', 'Florida', 'Florida State', 'Vanderbilt', 'Tennessee', 'Texas', 'Texas A&M', 'Wake Forest', 'North Carolina', 'Clemson', 'Virginia', 'Stanford', 'Oklahoma', 'Oklahoma State', 'Ole Miss', 'Mississippi State', 'TCU', 'Auburn', 'South Carolina'];
  ...
  for (const record of data) {
    const { score, tier, missing, autoVerified, autoVerifiedPartial } = scoreRecord(record);
    tierCounts[tier]++;
    ...
    if (costMatch) {
      ...
      const isTeam = costText.toLowerCase().includes('team') || detailsText.toLowerCase().includes('team');
      if (price > 0 && (price < 40 || (price >= 500 && isTeam))) {
        suspiciousPrices.push({ university: record.university, cost: costText, reason: price < 40 ? 'Too Low' : 'Likely Team' });
      }
    }
  }
  ...
  results.sort((a, b) => (b.prestige - a.prestige) || (a.score - b.score) || a.university.localeCompare(b.university));
  ...
  if (duplicates.length > 0) {
    ...
    duplicates.forEach(d => console.log(`   - [${d.url}] shared by: "${d.s1}" AND "${d.s2}"`));
  } else {
  ...
  if (suspiciousPrices.length > 0) {
    ...
    suspiciousPrices.forEach(p => console.log(`   - "${p.university}": ${p.cost}`));
  } else {
  ...
  for (const record of data) {
    ...
    if (record.autoVerified !== autoVerified || record.autoVerifiedPartial !== autoVerifiedPartial) {
  ...
  console.log(`🏷️  Stamped autoVerified flags on ${updated} records in camps_data.json\n`);
}
