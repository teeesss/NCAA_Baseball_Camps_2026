...
const {
  ...
  MASCOT_LOOKUP,
} = require("./src/utils/mascot_lookup.js");
const {
  isContaminated,
  isStrictBoundaryMatch,
} = require("./src/utils/contamination_check.js");
...
const SEARCH_ENGINES = [
  {
    name: "DuckDuckGo",
    url: (q) =>
      `https://duckduckgo.com/html/?q=${encodeURIComponent(q)}&ia=web`,
    selector: ".result__a",
    hrefAttr: "href",
  },
  {
...
function isOfficialPlatform(url) {
  ...
  try {
    ...
    const hasBaseballKeyword =
      hostname.includes("baseball") &&
      (hostname.includes("camp") || hostname.includes("clinic"));
    return (
      OFFICIAL_PLATFORMS.some((p) => hostname.includes(p)) ||
...
async function validatePage(page, url, school, allUniversities, options = {}) {
  try {
    ...
    if (options.isConsensus) {
      confidence += 40;
      reasons.push(
        `🎯 SEARCH CONSENSUS: Multiple engines identified this site`,
      );
    }
    ...
    for (const [uni, m] of Object.entries(MASCOT_LOOKUP)) {
      ...
      if (url.toLowerCase().includes(rivalM) && !combined.includes(mascot)) {
        confidence -= 80;
        reasons.push(
          `⚠️ CROSS-SCHOOL: URL contains mascot "${rivalM}" of ${uni}`,
        );
        break;
      }
    }
    ...
    if (
      ...
      !url.toLowerCase().includes(mascot)
    ) {
      if (!isOfficialPlatform(url)) {
        confidence -= 30;
        reasons.push(
          `⚠️ IDENTITY: Mascot "${mascot}" missing from page text and URL`,
        );
      } else {
        reasons.push(
          `ℹ️ Mascot "${mascot}" missing from text, but site is official athletics portal`,
        );
      }
    }
...
function extractDataFromText(text) {
  ...
  const isTeamCamp =
    lowerText.includes("team camp") && !lowerText.includes("individual");
  const isLegacy2025 =
    lowerText.includes("2025") && !lowerText.includes("2026");
  ...
  if (cleanEmails.length > 0) {
    result.email =
      cleanEmails.find((e) => e.endsWith(".edu")) || cleanEmails[0];
  }

  // POC / Contact names (Basic heuristics)
  const pocMatch = text.match(
    /(?:contact|coach|director|questions)\b[:\s-]*([A-Z][a-z]+ [A-Z][a-z]+)/,
  );
  if (pocMatch) {
      ...
      try {
        ...
        const mailtos = await page.evaluate(() =>
          Array.from(document.querySelectorAll('a[href^="mailto:"]')).map((a) =>
            a.href.replace("mailto:", "").split("?")[0].toLowerCase(),
          ),
        );
...
async function consensusSearch(
  ...
  const baseQuery = mascot
    ? `${school.university} ${mascot}`
    : school.university;
  ...
  if (isDeepMode) {
    ...
    log(
      `  🔍 Consensus search: "${searchTerm}"${excludeUrl ? " (Excluding previous)" : ""}`,
    );
  }
  ...
  for (const engine of engines) {
    try {
      log(`    → ${engine.name}...`);
      await page.goto(engine.url(searchTerm), {
        waitUntil: "domcontentloaded",
        timeout: 20000,
      });
      await humanDelay(1500, 3000);
      ...
      const cleaned = results
        ...
        .filter(
          (u) =>
            u &&
            !isBlacklisted(u) &&
            (!excludeUrl || !u.includes(new URL(excludeUrl).hostname)),
        );

      engineResults[engine.name] = cleaned;
      log(
        `    ✓ ${engine.name}: ${cleaned.length} results${cleaned[0] ? " → " + cleaned[0].substring(0, 60) : ""}`,
      );
    } catch (err) {
  ...
  function getUrlScore(domain, url) {
    ...
    if (
      domain.includes("baseball") &&
      (domain.includes("camp") || domain.includes("clinic"))
    ) {
    ...
    if (
      domain.endsWith(".edu") &&
      !lowerUrl.includes("baseball") &&
      !lowerUrl.includes("athletics") &&
  ...
  const ranked = Object.entries(engineHits).sort((a, b) => {
  ...
  if (ranked.length > 0) {
    ...
    log(
      `  🎯 Consensus: ${topDomain} (${topCount} engines agree) → ${confidence} confidence`,
    );
    return {
      url: bestUrl,
      confidence,
      domain: topDomain,
      engineAgreement: topCount,
    };
  }
  ...
  for (const urls of Object.values(engineResults)) {
    if (urls.length > 0) {
      log(
        `  ⚠️ No consensus — using best available: ${urls[0].substring(0, 60)}`,
      );
      return {
        url: urls[0],
        confidence: "LOW",
...
async function run() {
  ...
  if (!FORCE_REAUDIT && !SCHOOL_FILTER) {
    queue = queue.filter((s) => {
      ...
      if (
        ["URL_MISMATCH", "NO_DATA", "ERROR", "URL_BLACKLISTED"].includes(
          s.auditStatus,
        )
      )
        return true;
      ...
      const isThin =
        s.dates === "TBA" || !s.dates || s.cost === "TBA" || !s.cost;
  ...
  if (SCHOOL_FILTER) {
    const filters = (
      Array.isArray(SCHOOL_FILTER) ? SCHOOL_FILTER : [SCHOOL_FILTER]
    ...
    log(
      `🔎 School filter: [${Array.isArray(SCHOOL_FILTER) ? SCHOOL_FILTER.join(", ") : SCHOOL_FILTER}] → ${queue.length} match(es)`,
    );
  }
  ...
  if (BATCH_NUM) {
    ...
    log(
      `📦 Batch ${BATCH_NUM}: schools ${start + 1}-${Math.min(end, start + queue.length)}`,
    );
  }
  ...
  if (queue.length === 0) {
    log(
      "✅ No schools to process. All DI schools are audited or human-verified.",
    );
    return;
  }
  ...
  for (const school of queue) {
    ...
    log(`\n[${"─".repeat(60)}]`);
    log(
      `[${processed}/${queue.length}] ${school.university} (${school.conference || "N/A"})`,
    );
    ...
    try {
      ...
      if (
        FAST_MODE &&
        record.campUrl &&
        record.campUrl.includes("http") &&
        !isBlacklisted(record.campUrl)
      ) {
        log(`  🏃 FAST MODE: Using existing URL...`);
      } else {
        if (record.campUrl && isBlacklisted(record.campUrl)) {
          log(
            `  ℹ️ CLEANUP MODE: Existing URL is blacklisted (Junk/Aggregator). Initiating search for official portal...`,
          );
          record.campUrl = null;
        }
      ...
      if (searchResult && searchResult.engineAgreement >= 2) {
        log(
          `  ⭐ High Consensus Achieved (${searchResult.engineAgreement} engines). Prioritizing over database.`,
        );
      ...
      if (record.campUrl && record.campUrl.startsWith("http")) {
        ...
        if (isBlacklisted(record.campUrl)) {
          ...
          const isConsensus = !!(
            searchResult &&
            searchResult.engineAgreement >= 2 &&
            record.campUrl === searchResult.url
          );
          ...
          log(
            `  → Confidence: ${validation.confidence} | Valid: ${validation.valid}`,
          );
          ...
          if (validation.valid) {
            ...
            const extracted = await deepExtract(
              page,
              record.campUrl,
              school,
              allUniversities,
            ...
            const hasDates = !!(
              extracted.dates &&
              extracted.dates !== "TBA" &&
              !extracted.dates.includes("April 03, 2026")
            ); // Skip today false-positive
            const hasCost = !!(extracted.cost && extracted.cost !== "TBA");
            const isSpecializedPlatform = OFFICIAL_PLATFORMS.some((p) =>
              record.campUrl.toLowerCase().includes(p),
            );

            // If it's a generic university domain and we only have dates (no cost/poc), it's likely a false positive
            const isThinResults =
              !hasDates ||
              (!hasCost && !isSpecializedPlatform && !extracted.email);

            if (isThinResults) {
              log(
                `  ⚠️ Results too thin on "${record.campUrl}" (Dates: ${hasDates}, Cost: ${hasCost}). Falling back to search...`,
              );
              schoolReport.notes.push(
                "Valid school domain but data is thin (missing cost/email); triggering search fallback",
              );
              ...
              if (schoolReport.extractedData) {
                if (
                  schoolReport.extractedData.dates &&
                  schoolReport.extractedData.dates !== "TBA"
                )
                  record.dates = schoolReport.extractedData.dates;
                if (
                  schoolReport.extractedData.cost &&
                  schoolReport.extractedData.cost !== "TBA"
                )
                  record.cost = schoolReport.extractedData.cost;
                if (
                  schoolReport.extractedData.contact &&
                  schoolReport.extractedData.contact !== "TBA"
                )
                  record.contact = schoolReport.extractedData.contact;
                if (
                  schoolReport.extractedData.email &&
                  schoolReport.extractedData.email !== "TBA"
                )
                  record.email = schoolReport.extractedData.email;
              }
              ...
              if (extracted.campPOC) {
                ...
                extracted.email &&
                (!record.campPOC || record.campPOC.includes("@"))
              ) {
              ...
              if (
                extracted.address &&
                extracted.address !== "On-campus facility"
              ) {
              ...
              if (extracted.campTiers && extracted.campTiers.length > 0) {
                ...
                log(
                  `    📋 Camp Tiers: ${extracted.campTiers.length} items (Refreshed)`,
                );
                dataUpdated = true;
              }
            ...
            try {
              const badDomain = new URL(record.campUrl).hostname.replace(
                "www.",
                "",
              );
              newBlacklistDomains[badDomain] =
                (newBlacklistDomains[badDomain] || 0) + 1;
            } catch {}
      ...
      if (!initialUrlValidated && !record.campUrl) {
        log(
          `  ℹ️ Validation failed or yielded no data. Executing fallback consensus search...`,
        );
        searchResult = await consensusSearch(
          page,
          school,
          allUniversities,
          schoolReport.originalUrl,
        );
      }
      ...
      if (searchResult && !record.campUrl) {
        ...
        log(
          `  → Validation: confidence=${validation.confidence} valid=${validation.valid}`,
        );
        ...
        if (validation.valid) {
          ...
          schoolReport.confidence = validation.confidence;
          schoolReport.status =
            schoolReport.status === "URL_MISMATCH" ? "FIXED" : "FOUND";
          ...
          if (
            ...
            (!record.cost ||
              record.cost === "TBA" ||
              record.cost === "Contact for pricing")
          ) {
            record.cost = extracted.cost;
            log(`    💰 Cost: ${extracted.cost}`);
          }
          ...
          schoolReport.notes.push(
            `Search found ${searchResult.url} but validation failed: ${validation.reason}`,
          );
        }
      }
      ...
      if (!record.campUrl && !initialUrlValidated) {
        log(
          `  🕵️ Standard search failed. Attempting DEEP SEARCH with expanded depth...`,
        );
        const deepSearch = await consensusSearch(
          page,
          school,
          allUniversities,
          schoolReport.originalUrl,
        ...
        if (deepSearch && deepSearch.url) {
          const validation = await validatePage(
            ...
            school,
            allUniversities,
            { isConsensus: deepSearch.engineAgreement >= 2 },
          );
          if (validation.valid) {
            ...
            log(
              `  ❌ Deep search result failed validation: ${validation.reason}`,
            );
          }
        } else {
      ...
      if (!record.campUrl) {
        schoolReport.status =
          schoolReport.status === "NO_URL" ? "NO_DATA" : schoolReport.status;
      }
